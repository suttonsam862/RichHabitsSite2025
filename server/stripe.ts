...
// Helper function to create Shopify order from registration data
async function createShopifyOrderFromRegistration(
  ...
  try {
    console.log('Creating Shopify order with complete registration data:', {
      customer: `${registrationData.firstName} ${registrationData.lastName}`,
      email: registrationData.email,
      event: event.title,
      amount: amountPaid
    });

    // Create comprehensive order with all form field data
    const orderData = {
      order: {
        email: registrationData.email,
        ...
        tags: `Event Registration, ${event.title}, Online Payment`,
        customer: {
        ...
        line_items: [
          {
            ...
            properties: [
              ...
              { name: 'Registration Type', value: registrationData.registrationType },
              { name: 'Camper Name', value: `${registrationData.firstName} ${registrationData.lastName}` },
              { name: 'Parent/Guardian', value: registrationData.contactName },
              { name: 'Email', value: registrationData.email },
              { name: 'Phone', value: registrationData.phone || 'Not provided' },
              { name: 'Age', value: registrationData.age || 'Not provided' },
              { name: 'Grade', value: registrationData.grade || 'Not provided' },
              { name: 'Gender', value: registrationData.gender || 'Not provided' },
              { name: 'School', value: registrationData.schoolName || 'Not provided' },
              { name: 'Club', value: registrationData.clubName || 'Not provided' },
              { name: 'T-Shirt Size', value: registrationData.tShirtSize || 'Not provided' },
              { name: 'Day 1', value: registrationData.day1 ? 'Yes' : 'No' },
              { name: 'Day 2', value: registrationData.day2 ? 'Yes' : 'No' },
              { name: 'Day 3', value: registrationData.day3 ? 'Yes' : 'No' },
            ]
          }
        ],
        transactions: [
    ...
    console.log('Shopify order data prepared with all form fields:', JSON.stringify(orderData, null, 2));
    ...
    if (!response.ok) {
      ...
      throw new Error(`Shopify order creation failed: ${response.status} ${response.statusText}`);
    }
    ...
    // Don't throw error - allow registration to continue even if Shopify fails
    return null;
  }
}
