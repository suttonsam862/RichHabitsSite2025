...
/**
 * Direct Payment Intent Creation - Bypasses all problematic imports
 * Standalone implementation for Birmingham Slam Camp registration
 */
const directPaymentApp = express();

directPaymentApp.post("/api/events/1/create-payment-intent", async (req: Request, res: Response) => {
  try {
    console.log('Direct payment intent creation for Birmingham Slam Camp');
    ...
    console.error("Direct payment intent creation error:", error);
    res.status(500).json({ 
...
export function setupDirectPayment(app: any) {
  app.use(directPaymentApp);
}